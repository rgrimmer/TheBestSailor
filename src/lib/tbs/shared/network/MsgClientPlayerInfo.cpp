...
: MsgData(MsgType::ClientPlayerInfo)
, m_portUDP(0)
, m_name("") {

}

MsgClientPlayerInfo::MsgClientPlayerInfo(MsgData& message)
: MsgData(MsgType::ClientPlayerInfo) {
...
void MsgClientPlayerInfo::beforeOnSend(MsgData& message) {
//    (*this) << static_cast<sf::Uint16> (m_portUDP) << m_name;
    std::cout << "[Msg][OnSend] \t Send name(" << m_name << ") port(" << m_portUDP << ")" << std::endl;
}

void MsgClientPlayerInfo::afterOnReceive(MsgData& message) {
    sf::Uint16 sfPort;
    sf::Int32 t;
