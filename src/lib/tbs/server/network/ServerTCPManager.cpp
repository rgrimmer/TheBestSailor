...
void ServerTCPManager::initialize(unsigned short portTcp) {
    ...
    // Start listener tcp
    while (m_listener.listen(portTcp) != sf::Socket::Done) {
        std::cerr << "[TCP][Init][Err] \tCan't bind listener on TCP Server port. Retry in 5s." << std::endl;
...
bool ServerTCPManager::send(MsgData &message, sf::TcpSocket& player) const {
